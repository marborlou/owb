...
<div bind:this={itemElement} class="w-full {className} relative group" {draggable}>
	{#if confirmEdit}
		<div
			class=" w-full flex justify-between px-[11px] py-[6px] {id === $chatId || confirmEdit
				? 'bg-theme-chat dark:bg-gray-900'
				: selected
					? 'bg-gray-100 dark:bg-gray-950'
					: 'group-hover:bg-theme-chat/50 dark:group-hover:bg-gray-950'}  whitespace-nowrap text-ellipsis"
		>
			<input
				use:focusEdit
